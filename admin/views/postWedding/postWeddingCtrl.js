(function () {
    ...
    angular
        .module('emsAdmin')
        .controller('postWeddingCtrl', postWeddingCtrl);

    postWeddingCtrl.$inject = ['$scope', '$http', '$rootScope', 'localStorageService', 'toaster', '$state'];

    /* @ngInject */
    function postWeddingCtrl($scope, $http, $rootScope, localStorageService, toaster, $state) {
        var count=0;
        $scope.ldata = {};
        $scope.myFile = [];
        ...
        $scope.addImage = function (location, userForm) {
            ...
            if (userForm.$valid) {
                ...
                for (var index = 0; index < $scope.imageList1.length; index++) {
                    ...
                    $http.post($rootScope.ApiUrl + 'addPostWeddingImage', fd, {
        ...
        $scope.getImageList = function () {
            $http.get($rootScope.ApiUrl + 'postWeddingList').then(function (data) {
                if (data) {
        ...
        $scope.deleteImagerecord = function (customer) {
            $http.post($rootScope.ApiUrl + 'deletepostWeddingImage', customer).then(function (data) {
