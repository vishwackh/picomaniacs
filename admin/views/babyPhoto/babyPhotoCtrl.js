(function () {
    ...
    angular
        .module('emsAdmin')
        .controller('babyPhotoCtrl', babyPhotoCtrl);

    babyPhotoCtrl.$inject = ['$scope', '$http', '$rootScope', 'localStorageService', 'toaster', '$state'];

    /* @ngInject */
    function babyPhotoCtrl($scope, $http, $rootScope, localStorageService, toaster, $state) {
        var count=0;
        $scope.ldata = {};
        $scope.myFile = [];
        ...
        $scope.addImage = function (location, userForm) {
            ...
            if (userForm.$valid) {
                ...
                for (var index = 0; index < $scope.imageList1.length; index++) {
                    ...
                    $http.post($rootScope.ApiUrl + 'addBabyPhotoImage', fd, {
        ...
        $scope.getImageList = function () {
            $http.get($rootScope.ApiUrl + 'babyPhotoList').then(function (data) {
                if (data) {
        ...
        $scope.deleteImagerecord = function (customer) {
            $http.post($rootScope.ApiUrl + 'deletebabyPhotoImage', customer).then(function (data) {
