(function () {
    ...
    angular
        .module('emsAdmin')
        .controller('preWeddingCtrl', preWeddingCtrl);

    preWeddingCtrl.$inject = ['$scope', '$http', '$rootScope', 'localStorageService', 'toaster', '$state'];

    /* @ngInject */
    function preWeddingCtrl($scope, $http, $rootScope, localStorageService, toaster, $state) {
        var count=0;
        $scope.ldata = {};
        $scope.myFile = [];
        $scope.categories={'name':1,'name':2,'name':3};
        ...
        $scope.addImage = function (location, userForm) {
            ...
            if (userForm.$valid) {
                ...
                for (var index = 0; index < $scope.imageList1.length; index++) {
                    ...
                    $http.post($rootScope.ApiUrl + 'addPreWeddingImage', fd, {
        ...
        $scope.getImageList = function () {
            $http.get($rootScope.ApiUrl + 'preWeddingList').then(function (data) {
                if (data) {
        ...
        $scope.deleteImagerecord = function (customer) {
            $http.post($rootScope.ApiUrl + 'deletepreWeddingImage', customer).then(function (data) {
